...
namespace BingBackground
{
	public class BingBackgroundUpdater : IDisposable
	{
	    ...
	    public void Execute()
	    {
	        var timeoutSeconds = Properties.Settings.Default.TimeoutSeconds;
	        try
	        {
	            var succeed = UpdateBackgroundAsync(GetBackgroundUrlBase())
	                .Wait(timeoutSeconds * MILLISECONDS_IN_SECOND, this.cancellationTokenSource.Token);

	            if (succeed) log.Info("Update Completed!");
	            else log.Warn("Operation timed-out!");
	        }
	        ...
	        {
	            log.Error($"Failed updating background: {ex.Message}.", ex);
	        }
	    }
		...
		private static async Task<bool> WebsiteExistsAsync(string url)
		{
			try
			{
				...
				request.Timeout = Properties.Settings.Default.TimeoutSeconds * MILLISECONDS_IN_SECOND;
				request.Method = "HEAD";
		...
		private static async Task<string> GetResolutionExtensionAsync(string url)
		{
			...
			{
				log.Info($"Background for {widthByHeight} found.");
				return potentialExtension;
			}

			log.Info($"No background for {widthByHeight} was found. Using 1920x1080 instead.");
		...
		private static PicturePosition GetPosition()
		{
			...
			switch (Properties.Settings.Default.Position)
			{
				...
				case "Fill":
					...
					break;
				default:
					throw new NotSupportedException($"PicturePosition '{Properties.Settings.Default.Position}' is not supported.");
			}
